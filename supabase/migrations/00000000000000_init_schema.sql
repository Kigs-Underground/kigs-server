...
COMMENT ON TYPE "public"."author_type" IS 'Types of authors for posts. Can be a User or a page';
...
COMMENT ON TYPE "public"."event_attendance_status" IS 'Different statuses of attendance';
...
COMMENT ON TYPE "public"."event_type" IS 'Type of event based on start time and end time';



CREATE TYPE "public"."page_role" AS ENUM (
    'admin',
    'owner'
);


ALTER TYPE "public"."page_role" OWNER TO "postgres";


COMMENT ON TYPE "public"."page_role" IS 'Different roles for pages';
...
COMMENT ON TYPE "public"."page_type" IS 'Types of pages available on the platform';
...
CREATE TYPE "public"."post_type" AS ENUM (
    ...
    'youtube',
    'image',
    'notif_event_creation',
    'notif_mix_uploaded',
    'notif_friend_attending',
    'text',
    'link'
);
...
COMMENT ON TYPE "public"."post_type" IS 'Types of posts appearing in the feed';
...
COMMENT ON TYPE "public"."venue_type" IS 'Type of venue ';



CREATE OR REPLACE FUNCTION "public"."edit_event"("event_id" "uuid", "event_name" "text", "event_description" "text", "event_start_date" timestamp with time zone, "event_end_date" timestamp with time zone, "event_visual" "text", "artist_ids" "uuid"[]) RETURNS "json"
...
begin
  ...
  set 
    ...
    visual = event_visual
  where id = event_id
  returning to_json(events.*) into updated_event;

  -- Delete existing event_artist entries not in the new list
  delete from event_artist
  where event_id = edit_event.event_id
    and artist_id != all(edit_event.artist_ids);

  -- Insert new event_artist entries
  insert into event_artist (event_id, artist_id)
  select edit_event.event_id, unnest(edit_event.artist_ids)
  on conflict (event_id, artist_id) do nothing;

  return json_build_object(
    'event', updated_event,
    'artists', (select json_agg(to_json(pages.*)) from pages where id = any(edit_event.artist_ids))
  );
end;
$$;


ALTER FUNCTION "public"."edit_event"("event_id" "uuid", "event_name" "text", "event_description" "text", "event_start_date" timestamp with time zone, "event_end_date" timestamp with time zone, "event_visual" "text", "artist_ids" "uuid"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."find_nearest_city"("user_latitude" double precision, "user_longitude" double precision, "radius_miles" double precision) RETURNS TABLE("id" "uuid", "name" "text")
    LANGUAGE "plpgsql"
    AS $$
DECLARE
    earth_radius double precision := 3958.8; -- Radius of the Earth in miles
BEGIN
    RETURN QUERY
    SELECT c.id, c.name
...
ALTER FUNCTION "public"."find_nearest_city"("user_latitude" double precision, "user_longitude" double precision, "radius_miles" double precision) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_all_posts"() RETURNS TABLE("post_id" "uuid", "page_id" "uuid", "created_at" timestamp with time zone, "content" "text", "post_type" "text", "media_url" "text", "profile_picture" "text", "name" "text", "handle" "text", "like_count" integer, "comment_count" integer)
    LANGUAGE "plpgsql"
    AS $$
...
BEGIN
    ...
    SELECT 
        ...
        pg.name,
        pg.handle,
        COALESCE(CAST(likes_count.count AS INTEGER), 0) AS like_count,
        COALESCE(CAST(comments_count.count AS INTEGER), 0) AS comment_count
    FROM 
        posts p
    JOIN 
        pages pg ON p.page_id = pg.id
    LEFT JOIN 
        (SELECT post_id, COUNT(*) AS count FROM post_likes GROUP BY post_id) AS likes_count
        ON p.id = likes_count.post_id
    LEFT JOIN 
        (SELECT post_id, COUNT(*) AS count FROM post_comments GROUP BY post_id) AS comments_count
        ON p.id = comments_count.post_id
...
CREATE OR REPLACE FUNCTION "public"."get_artist_events_attended_by_fan"("artist_id" "uuid", "fan_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "start_date" timestamp with time zone, "venue_name" "text")
    LANGUAGE "plpgsql"
    AS $$
...
BEGIN
    ...
    JOIN event_fan ef ON e.id = ef.event_id
    JOIN event_venue ev ON e.id = ev.event_id
    JOIN pages v ON ev.venue_id = v.id  -- Assuming venue names are in 'pages'
    WHERE ea.artist_id = artist_id
      AND ef.fan_id = fan_id
      AND ef.attendance = 'going'  -- Fan attended
      AND e.start_date < NOW();  -- Past events only
END;
$$;


ALTER FUNCTION "public"."get_artist_events_attended_by_fan"("artist_id" "uuid", "fan_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_attended_events_by_artist_and_fan"("artist_id" "uuid", "fan_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "start_date" timestamp with time zone, "venue_name" "text")
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    RETURN QUERY
...
ALTER FUNCTION "public"."get_attended_events_by_artist_and_fan"("artist_id" "uuid", "fan_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_attended_venue_events_by_fan"("venue_id" "uuid", "fan_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "promoter_name" "text", "start_date" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
  ...
  SELECT e.id AS event_id, 
      ...
      e.start_date
  FROM event_fan ef
  JOIN event_venue ev ON ef.event_id = ev.event_id
  JOIN events e ON ef.event_id = e.id
  JOIN event_promoter ep ON ef.event_id = ep.event_id
  JOIN pages p ON ep.promoter_id = p.id
  WHERE ef.fan_id = fan_id
    AND ef.attendance = 'going'
    AND ev.venue_id = venue_id
    AND e.start_date < current_timestamp
    AND ep.id = (
        SELECT ep2.id 
        FROM event_promoter ep2
        WHERE ep2.event_id = ef.event_id
        ORDER BY ep2.id ASC
        LIMIT 1
    )
...
ALTER FUNCTION "public"."get_attended_venue_events_by_fan"("venue_id" "uuid", "fan_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_connections"("personal_page_id" "uuid") RETURNS TABLE("id" "uuid", "name" "text", "profile_picture" "text")
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  RETURN QUERY
  SELECT 
    CASE 
      WHEN c.requester_page_id = personal_page_id THEN c.requested_page_id
      ELSE c.requester_page_id
    END as id,
    p.name as name,
  ...
  WHERE (c.requester_page_id = personal_page_id OR c.requested_page_id = personal_page_id)
  AND c.status = 'accepted';
END;
$$;


ALTER FUNCTION "public"."get_connections"("personal_page_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_discussions_by_page_id"("page_id" "uuid") RETURNS TABLE("event_id" "uuid", "discussion_id" "uuid", "discussion_name" "text", "discussion_profile_picture" "text", "last_message_author" "text", "last_message_text" "text", "last_message_date" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_column
BEGIN
    ...
    SELECT 
        ...
        e.name AS discussion_name,
        e.visual AS discussion_profile_picture,
        p.name AS last_message_author,
        lm.last_message_text,
        lm.last_message_date
    FROM discussions d
    JOIN discussion_page dp ON d.id = dp.discussion_id
    JOIN events e ON d.event_id = e.id
    LEFT JOIN last_messages lm ON d.id = lm.discussion_id AND lm.rn = 1
    LEFT JOIN pages p ON lm.sender_page_id = p.id
    WHERE dp.participant_page_id = page_id;
END;
$$;


ALTER FUNCTION "public"."get_discussions_by_page_id"("page_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_event_card_details"("event_id" "uuid") RETURNS TABLE("id" "uuid", "event_name" "text", "event_visual" "text", "event_start_date" timestamp with time zone, "tickets_url" "text", "venue_name" "text", "promoter_name" "text", "promoter_profile_picture" "text")
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    RETURN QUERY
    SELECT 
        e.id,
        e.name AS event_name,
        e.visual AS event_visual,
        e.start_date AS event_start_date,
        e.tickets_url,
        (SELECT v.name
         FROM event_venue ev
         JOIN pages v ON ev.venue_id = v.id
        ...
        (SELECT p.name
         ...
         ORDER BY ep.id ASC
         LIMIT 1) AS promoter_name,
        (SELECT p.profile_picture
         FROM event_promoter ep
         JOIN pages p ON ep.promoter_id = p.id
         WHERE ep.event_id = e.id
         ORDER BY ep.id ASC
         LIMIT 1) AS promoter_profile_picture
    FROM events e
    WHERE e.id = event_id;
END;
$$;
...
ALTER FUNCTION "public"."get_event_card_details"("event_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_events"("city_id" "uuid", "event_types" "text"[]) RETURNS TABLE("id" "uuid", "name" "text", "start_date" "date", "description" "text", "venue_name" "text", "event_type" "text", "visual" "text", "artist_names" "text"[])
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    RETURN QUERY
    ...
    FROM events e
    LEFT JOIN event_venue ev ON ev.event_id = e.id
    LEFT JOIN pages v ON ev.venue_id = v.id
    LEFT JOIN event_artist ea ON e.id = ea.event_id
    LEFT JOIN pages a ON ea.artist_id = a.id
    WHERE ((e.start_date <= NOW() AND e.end_date >= NOW())  -- Include ongoing events
       OR (e.start_date > NOW()))  -- Include future events
    AND (
        EXISTS (
            SELECT 1
            FROM event_venue ev
            JOIN pages v ON ev.venue_id = v.id
            WHERE ev.event_id = e.id AND v.home_city_id = city_id
        )
        OR EXISTS (
            SELECT 1
            FROM event_promoter ep
            JOIN pages p ON ep.promoter_id = p.id
            WHERE ep.event_id = e.id AND p.home_city_id = city_id
        )
    )
    AND (array_length(event_types, 1) IS NULL OR e.event_type::text = any(event_types))  -- Filter by event types if not empty
...
ALTER FUNCTION "public"."get_events"("city_id" "uuid", "event_types" "text"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_following_feed"("user_page_id" "uuid", "page_limit" integer, "page_offset" integer) RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
    followed_ids uuid[];
    result_json jsonb;
BEGIN
    -- Get IDs of pages followed by the user
    SELECT array_agg(followed_page_id)
    INTO followed_ids
    FROM public.followers
    WHERE follower_page_id = user_page_id;

    -- If the user follows no one, return an empty array
    IF followed_ids IS NULL OR array_length(followed_ids, 1) IS NULL THEN
        RETURN '[]'::jsonb;
    END IF;

    -- Fetch posts from followed pages, join author info and counts, format as JSON
    ...
    FROM (
        SELECT jsonb_build_object(
            ...
            'page', jsonb_build_object(
                ...
                'page_type', author.page_type
            ),
            'likes_count', COALESCE(lk.count, 0),
            'comments_count', COALESCE(cm.count, 0),
            'reposts_count', COALESCE(rp.count, 0)
        ) as post_data
        FROM public.posts p
        JOIN public.pages author ON p.page_id = author.id
        LEFT JOIN (SELECT post_id, count(*) FROM public.post_likes GROUP BY post_id) lk ON lk.post_id = p.id
        LEFT JOIN (SELECT post_id, count(*) FROM public.post_comments GROUP BY post_id) cm ON cm.post_id = p.id
        LEFT JOIN (SELECT post_id, count(*) FROM public.post_reposts GROUP BY post_id) rp ON rp.post_id = p.id
        WHERE p.page_id = ANY(followed_ids)
        ORDER BY p.created_at DESC
        LIMIT page_limit
        OFFSET page_offset
    ) AS posts_with_details;

    -- Return the aggregated JSON array, or an empty array if no posts found
    RETURN COALESCE(result_json, '[]'::jsonb);
END;
$$;


ALTER FUNCTION "public"."get_following_feed"("user_page_id" "uuid", "page_limit" integer, "page_offset" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_following_feed"("user_page_id" "uuid", "page_limit" integer, "page_offset" integer) IS 'Fetches posts for a user''s following feed, including author details and interaction counts.';



CREATE OR REPLACE FUNCTION "public"."get_future_attended_events_from_fan"("fan_id" "uuid", "attendance" "text") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "venue_name" "text", "start_date" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
  RETURN QUERY
  SELECT e.id AS event_id, e.name AS event_name, e.visual, p.name as venue_name, e.start_date
  FROM event_fan ef
  JOIN event_venue ev ON ef.event_id = ev.event_id
  JOIN pages p ON ev.venue_id = p.id
  JOIN events e ON ef.event_id = e.id
  WHERE ef.fan_id = fan_id
...
ALTER FUNCTION "public"."get_future_attended_events_from_fan"("fan_id" "uuid", "attendance" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_future_events_by_artist"("artist_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "start_date" timestamp with time zone, "venue_name" "text")
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
    ...
    JOIN event_artist ea ON e.id = ea.event_id
    JOIN event_venue ev ON e.id = ev.event_id
    JOIN pages v ON ev.venue_id = v.id  -- Assuming venue is stored in 'pages'
    WHERE ea.artist_id = artist_id
      AND e.start_date > NOW();  -- Use NOW() to compare timestamps with time zone
END;
$$;


ALTER FUNCTION "public"."get_future_events_by_artist"("artist_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_future_events_by_promoter"("promoter_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "start_date" timestamp with time zone, "venue_name" "text")
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
    ...
    SELECT 
        ...
        v.name AS venue_name
    FROM events e
    JOIN event_promoter ep ON e.id = ep.event_id
    JOIN event_venue ev ON e.id = ev.event_id
    JOIN pages v ON ev.venue_id = v.id  -- Assuming venues are in 'pages'
    WHERE ep.promoter_id = promoter_id
      AND e.start_date > NOW();  -- Future events only
END;
$$;


ALTER FUNCTION "public"."get_future_events_by_promoter"("promoter_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_future_events_by_venue"("venue_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "venue_name" "text", "start_date" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
...
ALTER FUNCTION "public"."get_future_events_by_venue"("venue_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_going_events_from_fan"("fan_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "venue_name" "text", "start_date" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
...
ALTER FUNCTION "public"."get_going_events_from_fan"("fan_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_interested_events_from_fan"("fan_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "venue_name" "text", "start_date" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
...
ALTER FUNCTION "public"."get_interested_events_from_fan"("fan_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_past_events_by_artist"("artist_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "start_date" timestamp with time zone, "venue_name" "text")
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
...
ALTER FUNCTION "public"."get_past_events_by_artist"("artist_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_past_events_by_promoter"("promoter_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "start_date" timestamp with time zone, "venue_name" "text")
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
...
ALTER FUNCTION "public"."get_past_events_by_promoter"("promoter_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_past_events_by_venue"("venue_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "venue_name" "text", "start_date" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
...
ALTER FUNCTION "public"."get_past_events_by_venue"("venue_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_past_events_from_fan"("fan_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "venue_name" "text", "start_date" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
...
ALTER FUNCTION "public"."get_past_events_from_fan"("fan_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_post_comments"("post_id" "uuid") RETURNS TABLE("page_id" "uuid", "page_name" "text", "profile_picture" "text", "comment" "text", "created_at" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
    ...
    SELECT
      pages.id,
      pages.name, 
      pages.profile_picture, 
      post_comments.text AS comment_text, 
      post_comments.created_at
    FROM 
      post_comments
    JOIN 
      pages ON post_comments.page_id = pages.id
    WHERE 
...
ALTER FUNCTION "public"."get_post_comments"("post_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_posts"("personal_page_id" "uuid", "city_id" "uuid", "is_following" boolean) RETURNS TABLE("post_id" "uuid", "page_id" "uuid", "created_at" timestamp with time zone, "content" "text", "post_type" "text", "media_url" "text", "profile_picture" "text", "name" "text", "handle" "text")
    LANGUAGE "plpgsql"
    AS $$BEGIN
    RETURN QUERY
    SELECT 
    ...
    FROM 
        public.posts p
    JOIN 
        public.pages pg ON p.page_id = pg.id
    WHERE 
        (is_following IS FALSE OR EXISTS (
            SELECT 1 
            FROM public.followers f 
            WHERE f.follower_page_id = personal_page_id 
            AND f.followed_page_id = pg.id
        ))
        AND (pg.home_city_id = city_id OR city_id IS NULL)
    ORDER BY 
        p.created_at DESC;
END;$$;


ALTER FUNCTION "public"."get_posts"("personal_page_id" "uuid", "city_id" "uuid", "is_following" boolean) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_posts_by_author"("author_id" "uuid") RETURNS TABLE("post_id" "uuid", "page_id" "uuid", "created_at" timestamp with time zone, "content" "text", "post_type" "text", "media_url" "text", "profile_picture" "text", "name" "text", "handle" "text", "like_count" integer, "comment_count" integer)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_column
BEGIN
  ...
  SELECT 
    ...
    pages.profile_picture,
    pages.name AS name,
    pages.handle AS handle,
    COALESCE(CAST(likes_count.count AS INTEGER), 0) AS like_count,
    COALESCE(CAST(comments_count.count AS INTEGER), 0) AS comment_count
  FROM 
    posts p
  JOIN 
    pages ON p.page_id = pages.id
  LEFT JOIN (
    SELECT post_id, COUNT(*) AS count 
    FROM post_likes 
    GROUP BY post_id
  ) AS likes_count ON p.id = likes_count.post_id
  LEFT JOIN (
    SELECT post_id, COUNT(*) AS count 
    FROM post_comments 
    GROUP BY post_id
  ) AS comments_count ON p.id = comments_count.post_id
  WHERE
    p.page_id = author_id  -- Filter by author if provided
  ORDER BY 
    p.created_at DESC;
END;
$$;
...
ALTER FUNCTION "public"."get_posts_by_author"("author_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_posts_by_city"("city_id" "uuid") RETURNS TABLE("post_id" "uuid", "page_id" "uuid", "created_at" timestamp with time zone, "content" "text", "post_type" "text", "media_url" "text", "profile_picture" "text", "name" "text", "handle" "text", "like_count" integer, "comment_count" integer)
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_column
BEGIN
...
ALTER FUNCTION "public"."get_posts_by_city"("city_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_posts_by_followed_pages"("fan_id" "uuid") RETURNS TABLE("post_id" "uuid", "page_id" "uuid", "created_at" timestamp with time zone, "content" "text", "post_type" "text", "media_url" "text", "profile_picture" "text", "name" "text", "handle" "text")
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
    ...
    JOIN followers f ON pg.id = f.followed_page_id
    WHERE f.follower_page_id = fan_id
    ORDER BY p.created_at DESC;
END;
$$;


ALTER FUNCTION "public"."get_posts_by_followed_pages"("fan_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_promoter_events_attended_by_fan"("promoter_id" "uuid", "fan_id" "uuid") RETURNS TABLE("event_id" "uuid", "event_name" "text", "visual" "text", "start_date" timestamp with time zone, "venue_name" "text")
    LANGUAGE "plpgsql"
    AS $$
#variable_conflict use_variable
BEGIN
    ...
    JOIN event_fan ef ON e.id = ef.event_id
    JOIN event_venue ev ON e.id = ev.event_id
    JOIN pages v ON ev.venue_id = v.id  -- Assuming venues are stored in 'pages'
    WHERE ep.promoter_id = promoter_id
      AND ef.fan_id = fan_id
      AND ef.attendance = 'going'  -- Fan attended the event
      AND e.start_date < NOW();  -- Past events only
END;
$$;


ALTER FUNCTION "public"."get_promoter_events_attended_by_fan"("promoter_id" "uuid", "fan_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_venues_for_map"("city_id" "uuid", "personal_page_id" "uuid", "event_types" "text"[] DEFAULT '{}'::"text"[], "date" timestamp with time zone DEFAULT NULL::timestamp with time zone) RETURNS TABLE("id" "uuid", "name" "text", "home_city_id" "uuid", "latitude" numeric, "longitude" numeric, "map_marker" "text", "has_events_matching" boolean, "has_connections_attending" boolean)
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    RETURN QUERY
    SELECT 
        ...
        v.latitude,
        v.longitude,
        v.map_marker,  -- Include map_marker in the select statement
        EXISTS (
            SELECT 1
            FROM event_venue ev
            JOIN events e ON ev.event_id = e.id
            ...
            AND e.start_date > CURRENT_TIMESTAMP
            AND (date IS NULL OR e.start_date::date = date::date)
            AND (array_length(event_types, 1) IS NULL OR e.event_type::text = ANY(event_types))
        ) AS has_events_matching,
        EXISTS (
            SELECT 1
            FROM event_venue ev
            JOIN events e ON ev.event_id = e.id
            JOIN event_fan ef ON ef.event_id = e.id
            JOIN connections c ON (c.requester_page_id = personal_page_id OR c.requested_page_id = personal_page_id)
            WHERE ev.venue_id = v.id
            AND c.status = 'accepted'
            AND ef.attendance IN ('going', 'interested')
            AND e.start_date > CURRENT_TIMESTAMP
            AND (date IS NULL OR e.start_date::date = date::date)
            AND (array_length(event_types, 1) IS NULL OR e.event_type::text = ANY(event_types))
        ) AS has_connections_attending
    FROM venues v
    JOIN pages p ON v.id = p.id
    WHERE p.home_city_id = city_id;
END;
$$;


ALTER FUNCTION "public"."get_venues_for_map"("city_id" "uuid", "personal_page_id" "uuid", "event_types" "text"[], "date" timestamp with time zone) OWNER TO "postgres";
...
COMMENT ON TABLE "public"."artists" IS 'Table of artists';
...
CREATE TABLE IF NOT EXISTS "public"."cities" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "name" "text" NOT NULL,
    "latitude" double precision DEFAULT '43.5'::double precision NOT NULL,
    "longitude" double precision DEFAULT '45'::double precision NOT NULL
);
...
CREATE TABLE IF NOT EXISTS "public"."connections" (
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "requester_page_id" "uuid" NOT NULL,
    "requested_page_id" "uuid" NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."discussion_page" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "discussion_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "participant_page_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL
);


ALTER TABLE "public"."discussion_page" OWNER TO "postgres";


COMMENT ON TABLE "public"."discussion_page" IS 'List of participants to a discussion';
...
CREATE TABLE IF NOT EXISTS "public"."discussions" (
    ...
    "created_by_page_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL
);
...
COMMENT ON TABLE "public"."discussions" IS 'All discussions';
...
COMMENT ON TABLE "public"."events" IS 'Table that stores all the events on the platform';



COMMENT ON COLUMN "public"."events"."tickets_url" IS 'URL to buy tickets from';
...
CREATE TABLE IF NOT EXISTS "public"."pages" (
    ...
    "discogs" "text",
    "soundcloud" "text",
    "soundcloudUserID" "text",
    "followers" numeric DEFAULT '0'::numeric NOT NULL,
    "home_city_id" "uuid",
    "ra_id" "text",
    "updated_at" timestamp with time zone
);
...
COMMENT ON COLUMN "public"."pages"."handle" IS 'Unique handle of the profile';



COMMENT ON COLUMN "public"."pages"."cover_picture" IS 'UREL of the cover picture';



COMMENT ON COLUMN "public"."pages"."soundcloud" IS 'URL of the Soundcloud Profile';



COMMENT ON COLUMN "public"."pages"."soundcloudUserID" IS 'User ID of the Soundcloud Profile';



CREATE OR REPLACE VIEW "public"."discussions_with_events" WITH ("security_invoker"='on') AS
 SELECT "d"."id" AS "discussion_id",
   ...
   FROM (("public"."discussions" "d"
     ...
     JOIN "public"."pages" "p" ON (("d"."created_by_page_id" = "p"."id")));


ALTER TABLE "public"."discussions_with_events" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."event_artist" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."event_fan" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "event_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "fan_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "attendance" "public"."event_attendance_status" NOT NULL
);


ALTER TABLE "public"."event_fan" OWNER TO "postgres";


COMMENT ON TABLE "public"."event_fan" IS 'Records the attendance status of fans to an event';



CREATE TABLE IF NOT EXISTS "public"."event_label" (
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "label_id" "uuid",
    "event_id" "uuid",
    "updated_at" "date",
...
ALTER TABLE "public"."event_label" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."event_promoter" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "event_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "promoter_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "updated_at" timestamp with time zone
);


ALTER TABLE "public"."event_promoter" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."followers" (
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "follower_page_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "followed_page_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL
);
...
COMMENT ON TABLE "public"."venues" IS 'Table of venues';



COMMENT ON COLUMN "public"."venues"."map_marker" IS 'Circle image to render when the venue is displayed on the map';



COMMENT ON COLUMN "public"."venues"."venue_type" IS 'Type of venue';
...
CREATE OR REPLACE VIEW "public"."locatedvenue" AS
 SELECT "pages"."id",
    ...
    "venues"."longitude"
   FROM ("public"."venues"
     LEFT JOIN "public"."pages" ON (("pages"."id" = "venues"."id")));


ALTER TABLE "public"."locatedvenue" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."messages" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "discussion_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "sender_page_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "text" "text" NOT NULL
);
...
COMMENT ON TABLE "public"."messages" IS 'Messages of a discussion';
...
ALTER TABLE "public"."mixes" OWNER TO "postgres";


CREATE OR REPLACE VIEW "public"."pageview" AS
 SELECT "pages"."id",
   ...
   FROM ("public"."pages"
     LEFT JOIN "public"."cities" ON (("pages"."home_city_id" = "cities"."id")));
...
COMMENT ON TABLE "public"."post_reposts" IS 'Tracks which page reposted which post.';
...
COMMENT ON TABLE "public"."posts" IS 'Table of posts appearing in the feed';



COMMENT ON COLUMN "public"."posts"."media_url" IS 'URL of the media to load';



CREATE TABLE IF NOT EXISTS "public"."promoter_artist" (
    "promoter_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "artist_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL
);


ALTER TABLE "public"."promoter_artist" OWNER TO "postgres";
...
COMMENT ON TABLE "public"."promoters" IS 'Table of promoters';



CREATE TABLE IF NOT EXISTS "public"."record_label_artist" (
    "record_label_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "artist_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "id" "uuid" NOT NULL
);


ALTER TABLE "public"."record_label_artist" OWNER TO "postgres";
...
ALTER TABLE "public"."record_labels" OWNER TO "postgres";


COMMENT ON TABLE "public"."record_labels" IS 'Table of record labels';



CREATE TABLE IF NOT EXISTS "public"."user_page" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."users_expo_push_tokens" (
    "user_id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
ALTER TABLE "public"."users_expo_push_tokens" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."artists"
    ADD CONSTRAINT "artists_ra_id_key" UNIQUE ("ra_id");
...
ALTER TABLE ONLY "public"."discussion_page"
    ADD CONSTRAINT "discussion_page_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."event_fan"
    ADD CONSTRAINT "event_fan_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."event_label"
    ADD CONSTRAINT "event_label_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."event_artist"
    ADD CONSTRAINT "event_line_up_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."event_promoter"
...
ALTER TABLE ONLY "public"."events"
    ADD CONSTRAINT "events_ra_id_key" UNIQUE ("ra_id");
...
ALTER TABLE ONLY "public"."mixes"
    ADD CONSTRAINT "mixes_track_id_key" UNIQUE ("track_id");
...
ALTER TABLE ONLY "public"."pages"
    ADD CONSTRAINT "pages_ra_id_key" UNIQUE ("ra_id");
...
ALTER TABLE ONLY "public"."post_reposts"
    ADD CONSTRAINT "post_reposts_unique" UNIQUE ("post_id", "page_id");
...
ALTER TABLE ONLY "public"."promoters"
    ADD CONSTRAINT "promoters_ra_id_key" UNIQUE ("ra_id");



ALTER TABLE ONLY "public"."record_label_artist"
    ADD CONSTRAINT "record_label_artist_id_key" UNIQUE ("id");



ALTER TABLE ONLY "public"."record_label_artist"
    ADD CONSTRAINT "record_label_artist_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."record_labels"
    ADD CONSTRAINT "record_labels_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."event_venue"
    ADD CONSTRAINT "unique_event_id_venue_id" UNIQUE ("event_id", "venue_id");
...
ALTER TABLE ONLY "public"."users_expo_push_tokens"
    ADD CONSTRAINT "users_expo_push_tokens_pkey" PRIMARY KEY ("user_id");



ALTER TABLE ONLY "public"."venues"
    ADD CONSTRAINT "venues_id_key" UNIQUE ("id");
...
ALTER TABLE ONLY "public"."venues"
    ADD CONSTRAINT "venues_ra_id_key" UNIQUE ("ra_id");



CREATE UNIQUE INDEX "eventartistindex" ON "public"."event_artist" USING "btree" ("artist_id", "event_id");



CREATE UNIQUE INDEX "eventpromoterindex" ON "public"."event_promoter" USING "btree" ("event_id", "promoter_id");



CREATE UNIQUE INDEX "handleindex" ON "public"."pages" USING "btree" ("handle", "page_type");



CREATE OR REPLACE TRIGGER "Added To Group Chat" AFTER INSERT ON "public"."discussion_page" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ydtueqjlkhymqzjhxbdb.supabase.co/functions/v1/addedToGroupChat', 'POST', '{"Content-type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlkdHVlcWpsa2h5bXF6amh4YmRiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcwOTQ3OTE1MywiZXhwIjoyMDI1MDU1MTUzfQ.hiXOe13YKEo5r6Xza1FGPN7gcrVwd-FU0vp3DCT1gXI"}', '{}', '5000');



CREATE OR REPLACE TRIGGER "Connection Request Accepted" AFTER UPDATE ON "public"."connections" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ydtueqjlkhymqzjhxbdb.supabase.co/functions/v1/connectionRequestAccepted', 'POST', '{"Content-type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlkdHVlcWpsa2h5bXF6amh4YmRiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcwOTQ3OTE1MywiZXhwIjoyMDI1MDU1MTUzfQ.hiXOe13YKEo5r6Xza1FGPN7gcrVwd-FU0vp3DCT1gXI"}', '{}', '5000');



CREATE OR REPLACE TRIGGER "New Connection Request" AFTER INSERT ON "public"."connections" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ydtueqjlkhymqzjhxbdb.supabase.co/functions/v1/newConnectionRequest', 'POST', '{"Content-type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlkdHVlcWpsa2h5bXF6amh4YmRiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcwOTQ3OTE1MywiZXhwIjoyMDI1MDU1MTUzfQ.hiXOe13YKEo5r6Xza1FGPN7gcrVwd-FU0vp3DCT1gXI"}', '{}', '1000');



CREATE OR REPLACE TRIGGER "New Event From Followed Artist" AFTER INSERT ON "public"."event_artist" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ydtueqjlkhymqzjhxbdb.supabase.co/functions/v1/newEventFollowedArtist', 'POST', '{"Content-type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlkdHVlcWpsa2h5bXF6amh4YmRiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcwOTQ3OTE1MywiZXhwIjoyMDI1MDU1MTUzfQ.hiXOe13YKEo5r6Xza1FGPN7gcrVwd-FU0vp3DCT1gXI"}', '{}', '5000');



CREATE OR REPLACE TRIGGER "New Event From Followed Promoter" AFTER INSERT ON "public"."event_promoter" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ydtueqjlkhymqzjhxbdb.supabase.co/functions/v1/newEventFollowedPromoter', 'POST', '{"Content-type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlkdHVlcWpsa2h5bXF6amh4YmRiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcwOTQ3OTE1MywiZXhwIjoyMDI1MDU1MTUzfQ.hiXOe13YKEo5r6Xza1FGPN7gcrVwd-FU0vp3DCT1gXI"}', '{}', '5000');



CREATE OR REPLACE TRIGGER "New Event From Followed Venue" AFTER INSERT ON "public"."event_venue" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ydtueqjlkhymqzjhxbdb.supabase.co/functions/v1/newEventFollowedVenue', 'POST', '{"Content-type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlkdHVlcWpsa2h5bXF6amh4YmRiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcwOTQ3OTE1MywiZXhwIjoyMDI1MDU1MTUzfQ.hiXOe13YKEo5r6Xza1FGPN7gcrVwd-FU0vp3DCT1gXI"}', '{}', '5000');



CREATE OR REPLACE TRIGGER "New Like" AFTER INSERT ON "public"."post_likes" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ydtueqjlkhymqzjhxbdb.supabase.co/functions/v1/newLike', 'POST', '{"Content-type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlkdHVlcWpsa2h5bXF6amh4YmRiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcwOTQ3OTE1MywiZXhwIjoyMDI1MDU1MTUzfQ.hiXOe13YKEo5r6Xza1FGPN7gcrVwd-FU0vp3DCT1gXI"}', '{}', '5000');



CREATE OR REPLACE TRIGGER "New Message" AFTER INSERT ON "public"."messages" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ydtueqjlkhymqzjhxbdb.supabase.co/functions/v1/newMessage', 'POST', '{"Content-type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlkdHVlcWpsa2h5bXF6amh4YmRiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcwOTQ3OTE1MywiZXhwIjoyMDI1MDU1MTUzfQ.hiXOe13YKEo5r6Xza1FGPN7gcrVwd-FU0vp3DCT1gXI"}', '{}', '1000');



CREATE OR REPLACE TRIGGER "set_created_at_event_artist" BEFORE INSERT ON "public"."event_artist" FOR EACH ROW EXECUTE FUNCTION "public"."set_created_at"();



CREATE OR REPLACE TRIGGER "set_created_at_event_promoter" BEFORE INSERT ON "public"."event_promoter" FOR EACH ROW EXECUTE FUNCTION "public"."set_created_at"();



CREATE OR REPLACE TRIGGER "set_created_at_event_venue" BEFORE INSERT ON "public"."event_venue" FOR EACH ROW EXECUTE FUNCTION "public"."set_created_at"();



CREATE OR REPLACE TRIGGER "set_created_at_events" BEFORE INSERT ON "public"."events" FOR EACH ROW EXECUTE FUNCTION "public"."set_created_at"();



CREATE OR REPLACE TRIGGER "set_created_at_pages" BEFORE INSERT ON "public"."pages" FOR EACH ROW EXECUTE FUNCTION "public"."set_created_at"();



CREATE OR REPLACE TRIGGER "set_created_at_venues" BEFORE INSERT ON "public"."venues" FOR EACH ROW EXECUTE FUNCTION "public"."set_created_at"();



CREATE OR REPLACE TRIGGER "set_updated_at" BEFORE UPDATE ON "public"."event_artist" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();



CREATE OR REPLACE TRIGGER "set_updated_at" BEFORE UPDATE ON "public"."event_promoter" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();



CREATE OR REPLACE TRIGGER "set_updated_at" BEFORE UPDATE ON "public"."event_venue" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
CREATE OR REPLACE TRIGGER "set_updated_at" BEFORE UPDATE ON "public"."pages" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
ALTER TABLE ONLY "public"."connections"
    ADD CONSTRAINT "connections_requested_page_id_fkey" FOREIGN KEY ("requested_page_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."connections"
    ADD CONSTRAINT "connections_requester_page_id_fkey" FOREIGN KEY ("requester_page_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."discussion_page"
    ADD CONSTRAINT "discussion_page_discussion_id_fkey" FOREIGN KEY ("discussion_id") REFERENCES "public"."discussions"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."discussion_page"
    ADD CONSTRAINT "discussion_page_participant_page_id_fkey" FOREIGN KEY ("participant_page_id") REFERENCES "public"."pages"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."discussions"
    ADD CONSTRAINT "discussions_created_by_page_id_fkey" FOREIGN KEY ("created_by_page_id") REFERENCES "public"."pages"("id");



ALTER TABLE ONLY "public"."discussions"
    ADD CONSTRAINT "discussions_event_id_fkey" FOREIGN KEY ("event_id") REFERENCES "public"."events"("id");



ALTER TABLE ONLY "public"."event_artist"
    ADD CONSTRAINT "event_artist_artist_id_fkey" FOREIGN KEY ("artist_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."event_artist"
    ADD CONSTRAINT "event_artist_event_id_fkey" FOREIGN KEY ("event_id") REFERENCES "public"."events"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."event_fan"
    ADD CONSTRAINT "event_fan_event_id_fkey" FOREIGN KEY ("event_id") REFERENCES "public"."events"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."event_fan"
    ADD CONSTRAINT "event_fan_fan_id_fkey" FOREIGN KEY ("fan_id") REFERENCES "public"."pages"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."event_label"
    ADD CONSTRAINT "event_label_event_id_fkey" FOREIGN KEY ("event_id") REFERENCES "public"."events"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."event_label"
    ADD CONSTRAINT "event_label_label_id_fkey" FOREIGN KEY ("label_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."event_promoter"
    ADD CONSTRAINT "event_promoter_event_id_fkey" FOREIGN KEY ("event_id") REFERENCES "public"."events"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."event_promoter"
    ADD CONSTRAINT "event_promoter_promoter_id_fkey" FOREIGN KEY ("promoter_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."event_venue"
    ADD CONSTRAINT "event_venue_event_id_fkey" FOREIGN KEY ("event_id") REFERENCES "public"."events"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."event_venue"
    ADD CONSTRAINT "event_venue_venue_id_fkey" FOREIGN KEY ("venue_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."followers"
    ADD CONSTRAINT "followers_followed_page_id_fkey" FOREIGN KEY ("followed_page_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."followers"
    ADD CONSTRAINT "followers_follower_page_id_fkey" FOREIGN KEY ("follower_page_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."messages"
    ADD CONSTRAINT "messages_sender_page_id_fkey" FOREIGN KEY ("sender_page_id") REFERENCES "public"."pages"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."mixes"
    ADD CONSTRAINT "mixes_artist_id_fkey" FOREIGN KEY ("artist_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."mixes"
    ADD CONSTRAINT "mixes_promoter_id_fkey" FOREIGN KEY ("promoter_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."mixes"
    ADD CONSTRAINT "mixes_venue_id_fkey" FOREIGN KEY ("venue_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."pages"
    ADD CONSTRAINT "pages_home_city_id_fkey" FOREIGN KEY ("home_city_id") REFERENCES "public"."cities"("id");



ALTER TABLE ONLY "public"."post_comments"
    ADD CONSTRAINT "post_comments_page_id_fkey" FOREIGN KEY ("page_id") REFERENCES "public"."pages"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."post_likes"
    ADD CONSTRAINT "post_likes_page_id_fkey" FOREIGN KEY ("page_id") REFERENCES "public"."pages"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."post_reposts"
    ADD CONSTRAINT "post_reposts_page_id_fkey" FOREIGN KEY ("page_id") REFERENCES "public"."pages"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."posts"
    ADD CONSTRAINT "posts_page_id_fkey" FOREIGN KEY ("page_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."promoter_artist"
    ADD CONSTRAINT "public_promoter_residencies_artist_id_fkey" FOREIGN KEY ("artist_id") REFERENCES "public"."artists"("id");



ALTER TABLE ONLY "public"."promoter_artist"
    ADD CONSTRAINT "public_promoter_residencies_promoter_id_fkey" FOREIGN KEY ("promoter_id") REFERENCES "public"."promoters"("id");



ALTER TABLE ONLY "public"."record_label_artist"
    ADD CONSTRAINT "public_record_label_artists_artist_id_fkey" FOREIGN KEY ("artist_id") REFERENCES "public"."artists"("id");



ALTER TABLE ONLY "public"."record_label_artist"
    ADD CONSTRAINT "public_record_label_artists_record_label_id_fkey" FOREIGN KEY ("record_label_id") REFERENCES "public"."record_labels"("id");



ALTER TABLE ONLY "public"."user_page"
    ADD CONSTRAINT "user_page_page_id_fkey" FOREIGN KEY ("page_id") REFERENCES "public"."pages"("id") ON UPDATE CASCADE ON DELETE CASCADE;



ALTER TABLE ONLY "public"."users_expo_push_tokens"
    ADD CONSTRAINT "users_expo_push_tokens_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



CREATE POLICY "Allow public read access to events" ON "public"."event_venue" FOR SELECT USING (true);
...
CREATE POLICY "Allow public read access to events" ON "public"."mixes" FOR SELECT USING (true);



CREATE POLICY "Authenticated users can delete their own Expo push token" ON "public"."users_expo_push_tokens" FOR DELETE TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "Authenticated users can insert their Expo push token" ON "public"."users_expo_push_tokens" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "Authenticated users can read their own Expo push token" ON "public"."users_expo_push_tokens" FOR SELECT TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "Authenticated users can update their own Expo push token" ON "public"."users_expo_push_tokens" FOR UPDATE TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id")) WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



ALTER TABLE "public"."promoter_artist" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."record_label_artist" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."record_labels" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."users_expo_push_tokens" ENABLE ROW LEVEL SECURITY;
...
GRANT ALL ON FUNCTION "public"."edit_event"("event_id" "uuid", "event_name" "text", "event_description" "text", "event_start_date" timestamp with time zone, "event_end_date" timestamp with time zone, "event_visual" "text", "artist_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."edit_event"("event_id" "uuid", "event_name" "text", "event_description" "text", "event_start_date" timestamp with time zone, "event_end_date" timestamp with time zone, "event_visual" "text", "artist_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."edit_event"("event_id" "uuid", "event_name" "text", "event_description" "text", "event_start_date" timestamp with time zone, "event_end_date" timestamp with time zone, "event_visual" "text", "artist_ids" "uuid"[]) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."find_nearest_city"("user_latitude" double precision, "user_longitude" double precision, "radius_miles" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."find_nearest_city"("user_latitude" double precision, "user_longitude" double precision, "radius_miles" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."find_nearest_city"("user_latitude" double precision, "user_longitude" double precision, "radius_miles" double precision) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_artist_events_attended_by_fan"("artist_id" "uuid", "fan_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_artist_events_attended_by_fan"("artist_id" "uuid", "fan_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_artist_events_attended_by_fan"("artist_id" "uuid", "fan_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_attended_events_by_artist_and_fan"("artist_id" "uuid", "fan_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_attended_events_by_artist_and_fan"("artist_id" "uuid", "fan_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_attended_events_by_artist_and_fan"("artist_id" "uuid", "fan_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_attended_venue_events_by_fan"("venue_id" "uuid", "fan_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_attended_venue_events_by_fan"("venue_id" "uuid", "fan_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_attended_venue_events_by_fan"("venue_id" "uuid", "fan_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_connections"("personal_page_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_connections"("personal_page_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_connections"("personal_page_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_discussions_by_page_id"("page_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_discussions_by_page_id"("page_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_discussions_by_page_id"("page_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_event_card_details"("event_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_event_card_details"("event_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_event_card_details"("event_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_events"("city_id" "uuid", "event_types" "text"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."get_events"("city_id" "uuid", "event_types" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_events"("city_id" "uuid", "event_types" "text"[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_following_feed"("user_page_id" "uuid", "page_limit" integer, "page_offset" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_following_feed"("user_page_id" "uuid", "page_limit" integer, "page_offset" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_following_feed"("user_page_id" "uuid", "page_limit" integer, "page_offset" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_future_attended_events_from_fan"("fan_id" "uuid", "attendance" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_future_attended_events_from_fan"("fan_id" "uuid", "attendance" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_future_attended_events_from_fan"("fan_id" "uuid", "attendance" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_future_events_by_artist"("artist_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_future_events_by_artist"("artist_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_future_events_by_artist"("artist_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_future_events_by_promoter"("promoter_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_future_events_by_promoter"("promoter_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_future_events_by_promoter"("promoter_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_future_events_by_venue"("venue_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_future_events_by_venue"("venue_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_future_events_by_venue"("venue_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_going_events_from_fan"("fan_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_going_events_from_fan"("fan_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_going_events_from_fan"("fan_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_interested_events_from_fan"("fan_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_interested_events_from_fan"("fan_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_interested_events_from_fan"("fan_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_past_events_by_artist"("artist_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_past_events_by_artist"("artist_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_past_events_by_artist"("artist_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_past_events_by_promoter"("promoter_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_past_events_by_promoter"("promoter_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_past_events_by_promoter"("promoter_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_past_events_by_venue"("venue_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_past_events_by_venue"("venue_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_past_events_by_venue"("venue_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_past_events_from_fan"("fan_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_past_events_from_fan"("fan_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_past_events_from_fan"("fan_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_post_comments"("post_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_post_comments"("post_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_post_comments"("post_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_posts"("personal_page_id" "uuid", "city_id" "uuid", "is_following" boolean) TO "anon";
GRANT ALL ON FUNCTION "public"."get_posts"("personal_page_id" "uuid", "city_id" "uuid", "is_following" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_posts"("personal_page_id" "uuid", "city_id" "uuid", "is_following" boolean) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_posts_by_author"("author_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_posts_by_author"("author_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_posts_by_author"("author_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_posts_by_city"("city_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_posts_by_city"("city_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_posts_by_city"("city_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_posts_by_followed_pages"("fan_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_posts_by_followed_pages"("fan_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_posts_by_followed_pages"("fan_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_promoter_events_attended_by_fan"("promoter_id" "uuid", "fan_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_promoter_events_attended_by_fan"("promoter_id" "uuid", "fan_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_promoter_events_attended_by_fan"("promoter_id" "uuid", "fan_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_venues_for_map"("city_id" "uuid", "personal_page_id" "uuid", "event_types" "text"[], "date" timestamp with time zone) TO "anon";
GRANT ALL ON FUNCTION "public"."get_venues_for_map"("city_id" "uuid", "personal_page_id" "uuid", "event_types" "text"[], "date" timestamp with time zone) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_venues_for_map"("city_id" "uuid", "personal_page_id" "uuid", "event_types" "text"[], "date" timestamp with time zone) TO "service_role";
...
GRANT ALL ON TABLE "public"."discussion_page" TO "anon";
GRANT ALL ON TABLE "public"."discussion_page" TO "authenticated";
GRANT ALL ON TABLE "public"."discussion_page" TO "service_role";
...
GRANT ALL ON TABLE "public"."discussions_with_events" TO "anon";
GRANT ALL ON TABLE "public"."discussions_with_events" TO "authenticated";
GRANT ALL ON TABLE "public"."discussions_with_events" TO "service_role";



GRANT ALL ON TABLE "public"."event_artist" TO "anon";
GRANT ALL ON TABLE "public"."event_artist" TO "authenticated";
GRANT ALL ON TABLE "public"."event_artist" TO "service_role";



GRANT ALL ON TABLE "public"."event_fan" TO "anon";
GRANT ALL ON TABLE "public"."event_fan" TO "authenticated";
GRANT ALL ON TABLE "public"."event_fan" TO "service_role";



GRANT ALL ON TABLE "public"."event_label" TO "anon";
GRANT ALL ON TABLE "public"."event_label" TO "authenticated";
GRANT ALL ON TABLE "public"."event_label" TO "service_role";



GRANT ALL ON TABLE "public"."event_promoter" TO "anon";
GRANT ALL ON TABLE "public"."event_promoter" TO "authenticated";
GRANT ALL ON TABLE "public"."event_promoter" TO "service_role";



GRANT ALL ON TABLE "public"."event_venue" TO "anon";
GRANT ALL ON TABLE "public"."event_venue" TO "authenticated";
GRANT ALL ON TABLE "public"."event_venue" TO "service_role";
...
GRANT ALL ON TABLE "public"."locatedvenue" TO "anon";
GRANT ALL ON TABLE "public"."locatedvenue" TO "authenticated";
GRANT ALL ON TABLE "public"."locatedvenue" TO "service_role";
...
GRANT ALL ON TABLE "public"."mixes" TO "anon";
GRANT ALL ON TABLE "public"."mixes" TO "authenticated";
GRANT ALL ON TABLE "public"."mixes" TO "service_role";



GRANT ALL ON TABLE "public"."pageview" TO "anon";
GRANT ALL ON TABLE "public"."pageview" TO "authenticated";
GRANT ALL ON TABLE "public"."pageview" TO "service_role";
...
GRANT ALL ON TABLE "public"."promoter_artist" TO "anon";
GRANT ALL ON TABLE "public"."promoter_artist" TO "authenticated";
GRANT ALL ON TABLE "public"."promoter_artist" TO "service_role";
...
GRANT ALL ON TABLE "public"."record_label_artist" TO "anon";
GRANT ALL ON TABLE "public"."record_label_artist" TO "authenticated";
GRANT ALL ON TABLE "public"."record_label_artist" TO "service_role";



GRANT ALL ON TABLE "public"."record_labels" TO "anon";
GRANT ALL ON TABLE "public"."record_labels" TO "authenticated";
GRANT ALL ON TABLE "public"."record_labels" TO "service_role";



GRANT ALL ON TABLE "public"."user_page" TO "anon";
GRANT ALL ON TABLE "public"."user_page" TO "authenticated";
GRANT ALL ON TABLE "public"."user_page" TO "service_role";



GRANT ALL ON TABLE "public"."users_expo_push_tokens" TO "anon";
GRANT ALL ON TABLE "public"."users_expo_push_tokens" TO "authenticated";
GRANT ALL ON TABLE "public"."users_expo_push_tokens" TO "service_role";
