...
console.log('New Connection Request Event Triggered')
...
Deno.serve(async (req) => {
    ...
    // Get requester's page details
    const { data: requesterPage } = await supabase
        .from('pages')
        .select('name')
        .eq('id', payload.record.requester_page_id)
        .single()

    console.log('Looking for users of page: ' + payload.record.requested_page_id)

    const { data: pageUsers } = await supabase
        .from('user_page')
        .select('user_id')
        .eq('page_id', payload.record.requested_page_id)
    ...
    const messages = userTokens.map(token => {
        ...
        return {
            ...
            body: `${requesterPage?.name || 'Someone'} has sent you a connection request`,
        }
