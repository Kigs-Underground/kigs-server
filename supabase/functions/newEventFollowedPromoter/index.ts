...
interface WebhookPayload {
  ...
  table: string
  record: EventPromoter
  schema: 'public'
  old_record: null | EventPromoter
}
...
Deno.serve(async (req) => {
    ...
    const { event_id, promoter_id } = payload.record
    console.log('Processing event_promoter - Event:', event_id, 'Promoter:', promoter_id)
    
    // Get event details
    // Get event details and promoter page name
    console.log('Fetching event and promoter details')
    const [{ data: event }, { data: promoterPage }] = await Promise.all([
    supabase
        .from('events')
        .select('name')
        .eq('id', event_id)
    ...
    if (!promoterPage) {
        console.log('Promoter page not found, exiting')
        return new Response('Promoter page not found', { status: 200 })
    }
    console.log('Found event:', event.name)
    console.log('Found promoter:', promoterPage.name)

    // Get all followers of this promoter
    console.log('Fetching followers for promoter:', promoter_id)
    const { data: followers } = await supabase
        .from('followers')
        .select('follower_page_id')
        .eq('followed_page_id', promoter_id)
    ...
    const messages = userTokens.map(token => {
        ...
        return {
            to: token,
            sound: 'default',
            title: `Just announced from ${promoterPage.name}`,
            body: `${event.name}`,
        }
    }).filter(Boolean)
