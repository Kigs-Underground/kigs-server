...
Deno.serve(async (req) => {
    ...
    const { event_id, venue_id } = payload.record
    console.log('Processing event_venue - Event:', event_id, 'Venue:', venue_id)
    
    // Get event details
    // Get event details and promoter page name
    console.log('Fetching event and venue details')
    const [{ data: event }, { data: venuePage }] = await Promise.all([
    supabase
        .from('events')
        .select('name')
        .eq('id', event_id)
    ...
    if (!venuePage) {
        console.log('Venue page not found, exiting')
        return new Response('Venue page not found', { status: 200 })
    }
    console.log('Found event:', event.name)
    console.log('Found venue:', venuePage.name)

    // Get all followers of this promoter
    console.log('Fetching followers for venue:', venue_id)
    const { data: followers } = await supabase
        .from('followers')
        .select('follower_page_id')
        .eq('followed_page_id', venue_id)
    ...
    const messages = userTokens.map(token => {
        ...
        return {
            to: token,
            sound: 'default',
            title: `Just announced from ${venuePage.name}`,
            body: `${event.name}`,
        }
    }).filter(Boolean)
